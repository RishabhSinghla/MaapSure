...
export default function AuditPage() {
  ...
  return <><ErrorNotice>{error}</ErrorNotice><section className={`panel audit-integrity ${data.integrity.valid ? 'valid' : 'invalid'}`}><span><ShieldCheck /></span><div><small>Tamper-evident audit ledger</small><h2>{data.integrity.valid ? 'Every audit link is valid' : 'Integrity failure detected'}</h2><p>{data.integrity.checked} permanent event(s) checked from genesis to the current ledger head.</p></div><code>{data.integrity.headHash || data.integrity.brokenAt}</code></section>
    <section className="panel reports-panel"><div className="panel-heading"><div><span className="eyebrow"><Link2 size={14} /> SHA-256 chained history</span><h3>Permanent activity trail</h3></div><StatusBadge status={data.integrity.valid ? 'Verified' : 'Broken'} /></div><div className="table-scroll"><table><thead><tr><th>Sequence</th><th>Action</th><th>Actor</th><th>Target</th><th>Time</th><th>Fingerprint</th></tr></thead><tbody>{data.events.map((event) => <tr key={event.id}><td>#{event.sequence}</td><td><strong>{event.action}</strong></td><td>{event.actor.name}<small>{event.actor.role}</small></td><td>{event.targetType}<small>{event.targetId || '-'}</small></td><td>{new Date(event.at).toLocaleString('en-IN')}</td><td><span className="fingerprint"><Fingerprint size={13} /> {event.hash.slice(0, 14)}...</span></td></tr>)}</tbody></table></div></section></>;
}
