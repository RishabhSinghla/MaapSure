  ...
  return (
    <div className="test-wizard">
      ...
      {step === 3 && evaluation && <section className="panel wizard-panel result-review">
        ...
        <div className="review-grid"><div><span className="eyebrow">Core checks</span>{Object.values(evaluation.sections).map((section) => <article className="review-check" key={section.name}><span className={section.passed ? 'pass-dot' : 'fail-dot'}>{section.passed ? <Check /> : '!'}</span><div><strong>{section.name}</strong><small>{section.summary}</small></div><em>{section.passed ? 'PASS' : 'FAIL'}</em></article>)}</div><div className="diagnostic-card"><span className="eyebrow"><Sparkles size={14} /> Explainable diagnostic review</span><div className={`risk-banner ${evaluation.diagnostic.risk.toLowerCase()}`}>Risk level: <strong>{evaluation.diagnostic.risk}</strong></div>{evaluation.diagnostic.findings.map((finding) => <article key={finding.title}><i className={finding.severity} /><div><strong>{finding.title}</strong><p>{finding.detail}</p></div></article>)}</div></div>
        ...
        <div className="wizard-footer"><button className="button secondary" onClick={() => setStep(2)}><ArrowLeft size={18} /> Change readings</button><button className="button primary" disabled={busy} onClick={finalize}>{busy ? 'Creating report...' : 'Finalize and create report'} <ArrowRight size={18} /></button></div>
      </section>}
    </div>
  );
}
