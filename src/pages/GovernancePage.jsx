...
export default function GovernancePage() {
  ...
  return <div className="governance-grid"><ErrorNotice>{error}</ErrorNotice>
    ...
    <section className="panel governance-card"><div className="panel-heading"><div><span className="eyebrow">Controlled scope</span><h3>What this version governs</h3></div><LockKeyhole /></div><div className="governance-body"><p>{active.jurisdictionNote}</p><div className="scope-counts"><div><strong>{data.reference.reportSections.length}</strong><span>officially ordered report sections</span></div><div><strong>{data.reference.requirementFamilies.length + data.reference.detailedChecklistCount}</strong><span>clause families and detailed checklist rows</span></div></div><p>{data.reference.coverageNotice}</p><p className="method-note">Published rules are immutable. An approved report retains the rules-content fingerprint used for its calculation.</p></div></section>
